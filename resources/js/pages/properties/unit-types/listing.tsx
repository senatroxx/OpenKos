...
export default function Listing({
    ...
    const toggle = () =>
        router.patch(
            properties.unitTypes.publication.update({ property, unitType }),
            { is_published: !listing?.is_included },
            { preserveScroll: true },
        );
        ...
        >
            <div className="max-w-3xl space-y-4">
                <section className="space-y-4 rounded-lg border bg-card p-4 sm:p-5">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Publication and readiness use the existing Unit
                                Type listing rules.
                            </p>
                        </div>
                        <Badge
                    ...
                    {canManage && (
                        <Button
                            disabled={!ready && !listing?.is_included}
                            onClick={toggle}
                        >
                            {listing?.is_included
                                ? 'Remove from listing'
                    ...
                    {listing?.is_included && previewUrl && (
                ...
                <section className="space-y-3 rounded-lg border bg-card p-4 sm:p-5">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Manage Unit Type details, amenities, and photos from the
                        edit flow.
                    </p>
                    {canManage && (
                        <Button
