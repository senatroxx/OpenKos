...
const billingUnitLabels: Record<UnitTypeRate['billing_unit'], string> = {
    day: 'Daily',
    week: 'Weekly',
    month: 'Monthly',
    year: 'Yearly',
...
function formatRatePeriod(
    interval: number,
    unit: UnitTypeRate['billing_unit'],
): string {
    return interval === 1
        ? t(billingUnitLabels[unit])
        : `${t('Every')} ${interval} ${t(`${unit}s`)}`;
}
...
export default function UnitTypeRates({
    ...
    const [newRate, setNewRate] = useState<NewUnitTypeRateData>({
        amount: '',
    ...
    function submitRates(rates: UnitTypeRate[], onSuccess?: () => void): void {
        if (processing) {
            return;
        }

        transform((formData) => ({ ...formData, rates }));
        submit(properties.unitTypes.rates.update({ property, unitType }), {
            preserveState: true,
            onSuccess: (page) => {
                transform((formData) => formData);
                const updatedUnitType = page.props.unitType as
                    | UnitTypeRatesPageProps['unitType']
                    | undefined;
                setData((current) => ({
                    ...current,
                    rates: updatedUnitType?.rates ?? current.rates,
                    updated_at:
                        updatedUnitType?.updated_at ?? current.updated_at,
                }));
                onSuccess?.();
            },
    ...
    function editRate(rate: UnitTypeRate): void {
        if (rate.id == null) {
            return;
        }
    ...
    function saveEdit(rate: UnitTypeRate): void {
        if (rate.id == null) {
            return;
        }
    ...
    function toggleRate(rate: UnitTypeRate): void {
        if (rate.id == null) {
            return;
        }
    ...
    function handleAddRate(event: React.FormEvent<HTMLFormElement>): void {
    ...
    function renderRateRow(rate: UnitTypeRate) {
    ...
    function handleAddDialogChange(open: boolean): void {
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            {t(
                                "Compare and manage this Unit Type's pricing variants.",
                            )}
                        </p>
                    </div>
                    <Button
            ...
            <Dialog open={addDialogOpen} onOpenChange={handleAddDialogChange}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {t(
                                'Add a currency and billing-period variant for this Unit Type.',
                            )}
                        </DialogDescription>
                    </DialogHeader>
