    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <Badge className={`${STATUS_COLORS[unit.status] ?? 'bg-gray-400'} text-white`}>
                    {STATUS_LABELS[unit.status] ?? unit.status}
                </Badge>
            </div>

            <div>
                <p className="mb-2 text-xs font-medium tracking-wider text-muted-foreground uppercase">Unit Details</p>
                <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-lg border bg-muted/30 p-4">
                        <p className="text-xs text-muted-foreground">Floor</p>
                        <p className="mt-1 text-sm font-medium">{unit.floor ?? '—'}</p>
                    </div>
                    <div className="rounded-lg border bg-muted/30 p-4">
                    ...
                    <div className="rounded-lg border bg-muted/30 p-4">
                        ...
                        <p className="mt-1 text-sm font-medium tabular-nums">{unit.capacity}</p>
                    </div>
                    <div className="rounded-lg border bg-muted/30 p-4">
            ...
            {unit.description && (
                <div>
                    ...
                    <p className="rounded-lg border bg-muted/30 p-4 text-sm whitespace-pre-wrap">{unit.description}</p>
                </div>
            )}
            ...
            {unit.notes && (
                <div>
                    ...
                    <p className="rounded-lg border bg-muted/30 p-4 text-sm whitespace-pre-wrap">{unit.notes}</p>
                </div>
            )}
        </div>
    );
