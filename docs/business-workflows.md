...
- `GenerateInvoices` materializes Pending invoices for active leases up to a 2-month horizon (idempotent via a unique `(lease_id, period_start)` index). It also runs synchronously inside `CreateLease`, `RenewLease`, and the `MoveOutLease` transfer path so invoices exist immediately. Each created invoice dispatches `Invoice\InvoiceGenerated` after commit (plugin hook).
