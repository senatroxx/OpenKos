...
3. **Consumers** — `app-sidebar.tsx` appends `platform.navigation.main`/`.footer` after the hardcoded items; `layouts/settings/layout.tsx` appends `platform.settings`; every entity workspace layout renders its tab strip through the shared `components/shared/workspace-tabs.tsx`, which appends platform tabs.
...
`ExamplePlugin` (gated by `OPENKOS_EXAMPLE_PLUGIN`, default on — set `false` in production) demonstrates a sidebar nav item, a settings nav entry, and a `workspace-header-badge` region component; its source shows a commented workspace-tab registration.
